...
class RotaryPositionalEmbedding(nn.Module):
    def __init__(self, theta: float, d_k: int, max_seq_len: int, device=None):
        ...
        self.device = device
        index = torch.arange(0, max_seq_len, device=device).reshape(-1, 1)
        exponent = torch.arange(start=0, end=d_k, step=2, device=device) / d_k
    ...
    def forward(self, X: torch.Tensor, token_positions: torch.Tensor):
        ...
        # return ret
        '''
        suppose X is {x1, x2, x3 .... xn},
        after RoPE, X -> Y, {y1, y2, y3...yn} 
        original calc is R = [[cos, -sin],[sin, cos]] @ [x1, x2].T which means
        y1 = x1 * cos - x2 * sin, y2 = x1 * sin + x2 * cos, seamless transformation applied to [x3, x4] -> [y3, y4], [x5, x6] -> [y5,y6]..
        this is same as [x1, x2] @ [[cos, -sin],[sin, cos]].T = [x1, x2] @ [[cos, sin],[-sin, cos]]
        then, split X into X1 X2, X1 = [x1, x3, x5...] X2 = [x2, x4, xn]
        X1 * cos = [x1 * cos, x3 * cos, ...], X2 * cos = [x2 * cos, x4 * cos, ...]
        X1 * sin = [x1 * sin, x3 * sin, ...], X2 * sin = [x2 * sin, x3 * sin, ...]
        we can get Y1 = X1 * cos - X2 * sin, Y2 = X1 * sin + X2 * cos, Y1 = [y1, y3, y5...], Y2 = [y2, y4, y6]
        torch.stack([Y1, Y2], -1) -> [[[y1, y2], [y3,y4], ...]]
        so need to flattern last 2 dimension
        '''
        # update, since generating will also use positoin embbeding, and we should support a long sequence, so update max_seq_len if needed
        if X.shape[-2] > self.max_seq_len:
            print('RotaryPositionalEmbedding: reach max_seq_len, multiple max_seq_len by 2!')
            self.__init__(self.theta, self.d_k, self.max_seq_len * 2, self.device)
...
# Y = rms_norm(Y)
# print(Y)
# net = transformer_lm(100, 12, 3, 64, 8, 160, 10000)
